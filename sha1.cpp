...
string sign( string message, string key )
{
	...
	{
		unsigned char letter = message[i];

		unsigned char x_or = key[j];

		unsigned char signedLetter = letter ^ x_or;

		signedMessage += signedLetter;
	}

	return signedMessage;
}

string verify(unsigned int signedMessage, string key, unsigned int length )
{

	string unsignedMessage = "";

	for ( int i=0, j=0; i<length; i++, j=(j+1)%key.length() )
	{
		//unsigned int letter = signedMessage[i];

		unsigned char x_or = key[j];


		unsigned int signedLetter = signedMessage ^ x_or;


		unsignedMessage += char(signedLetter);

	}
	return unsignedMessage;
}

unsigned int sign2( string message, string key )
{
	...
	for ( int i=0, j=0; i<message.length(); i++, j=(j+1)%key.length() )
	{
		...
		unsigned int signedLetter = letter ^ x_or;
		signedMessage += signedLetter;
	}
